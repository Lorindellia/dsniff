...
trigger_dump(void)
{
	...
	fprintf(f, "# $Id: trigger.c,v 1.19 2000/11/19 21:44:29 dugsong Exp $\n"
...
trigger_ip(struct libnet_ipv4_hdr *ip)
{
	...
	if ((len = t->decode->dc_func(buf, len, (u_char*)obuf, sizeof(obuf))) > 0) {
		record(ip->ip_src.s_addr, ip->ip_dst.s_addr, ip->ip_p,
		       0, 0, t->decode->dc_name, (u_char*)obuf, len);
	}		
}
...
trigger_udp(struct libnet_ipv4_hdr *ip)
{
	...
	if (ip->ip_p != IPPROTO_UDP || len < (int)sizeof(*udp))
		return;
	...
	if ((len = t->decode->dc_func(buf, len, (u_char*)obuf, sizeof(obuf))) > 0) {
		record(ip->ip_src.s_addr, ip->ip_dst.s_addr, IPPROTO_UDP,
		       ...
		       t->decode->dc_name, (u_char*)obuf, len);
	}
}
...
trigger_tcp_raw(struct libnet_ipv4_hdr *ip)
{
	...
	if (ip->ip_p != IPPROTO_TCP || len < (int)sizeof(*tcp))
		return;
	...
	if (t != NULL) {
		...
		len = t->decode->dc_func((u_char*)iov->iov_base, iov->iov_len,
					 (u_char*)obuf, sizeof(obuf));
