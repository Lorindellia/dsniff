...
#define DEFAULT_PCAP_FILTER ((char*)"tcp port 80 or port 8080 or port 3128")
...
timestamp(void)
{
	...
	if (len < 0 || len > (int)sizeof(tstr) - 5)
		return (NULL);
...
void
sniff_http_client(struct tcp_stream *ts, void **param)
{
	int i;
  (void) param;
	...
	case NIDS_DATA:
		if (ts->server.count_new != 0) {
			i = process_http_request(&ts->addr, (u_char*)ts->server.data,
						 ts->server.count -
						 ts->server.offset);
			nids_discard(ts, i);
		}
	...
	default:
		if (ts->server.count != 0) {
			process_http_request(&ts->addr, (u_char*)ts->server.data,
					     ts->server.count -
					     ts->server.offset);
		}
		break;
...
main(int argc, char *argv[])
{
  ...
  struct nids_chksum_ctl ctl;
    ...
    nids_params.pcap_filter = DEFAULT_PCAP_FILTER;
  }
